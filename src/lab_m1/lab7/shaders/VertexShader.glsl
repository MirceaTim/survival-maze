...
void main()
{
    ...
    float diffuse_light = 0.75 * material_kd * max(dot(world_normal, L), 0);

    // TODO(student): Compute specular light component
    int primesteLumina;
    if(diffuse_light > 0) primesteLumina = 1;
    else primesteLumina = 0;
    float specular_light = material_ks * 0.75 * primesteLumina * pow(max(dot(V, R), 0), material_shininess);
    ...
    float atenuare = 1 / (1 + 0.2 * distance(light_position, world_pos) + 0.12 * pow(distance(light_position, world_pos), 2));
