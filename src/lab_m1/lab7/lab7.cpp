...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int light = glGetUniformLocation(shader->GetProgramID(), "light_position");
    glUniform3fv(light, 1, glm::value_ptr(lightPosition));


    glm::vec3 eyePosition = GetSceneCamera()->m_transform->GetWorldPosition();
    // TODO(student): Set eye position (camera position) uniform
    int eye = glGetUniformLocation(shader->GetProgramID(), "eye_position");
    glUniform3fv(eye, 1, glm::value_ptr(eyePosition));

    // TODO(student): Set material property uniforms (shininess, kd, ks, object color)
    int shininess = glGetUniformLocation(shader->GetProgramID(), "material_shininess");
    int kd = glGetUniformLocation(shader->GetProgramID(), "material_kd");
    int ks = glGetUniformLocation(shader->GetProgramID(), "material_ks");
    int colour = glGetUniformLocation(shader->GetProgramID(), "object_color");

    glUniform1i(shininess, materialShininess);
    glUniform1f(kd, materialKd);
    glUniform1f(ks, materialKs);
    glUniform3fv(colour, 1, glm::value_ptr(color));
